...
class LocalStorageService {
  ...
  // In a real implementation, we would check permissions
  createRequest(requestData, userRole) {
    /*
    // Check permissions
  ...
  getDashboardStats() {
    ...
    const pendingRequests = data.requests.filter(r => {
      const status = data.requestStatuses.find(s => s.id === r.statusId);
      return status && status.status === 'PENDING';
    }).length;
    
    const occupiedRooms = data.rooms.filter(r => r.status !== 'CLEAN').length;
    const availableRooms = data.rooms.filter(r => r.status === 'CLEAN').length;
  ...
  getRecentActivity() {
    ...
    const recentRoomChanges = [...data.rooms]
    ...
    const recentActivity = [...recentRequests, ...recentRoomChanges]
